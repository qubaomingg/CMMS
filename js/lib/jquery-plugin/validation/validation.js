...
var past_value = null;
$.fn.validation = function (options , error , description) {
...
function validation_check(el , rules , options , error) {
        ...
        if (function_exist == 'function') {
                ...
                if( options.rule == 'describe' ){
                         ...
                         return check_spec( el , 200 , true , error );
                }
                ...
                if( options.rule == 'email' || options.rule == 'nick' || options.rule == 'lostpwd_email' || options.rule == 'domain' ){
                        ...
                        if ( past_value != validation_value || true ){
                            // @todo 检测用户名对不.
                            /*$.POST( 
                                "/reg/has_this_"+ ajax_validation_type +"_reg",
                                { 
                                    ajax_validation_type:validation_value
                                } , 
                                function(msg){
                                    console.log('username return');
                                    if( msg.is_unique == 'TRUE' ){
